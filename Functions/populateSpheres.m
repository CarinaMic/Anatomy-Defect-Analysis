...
TR = stlread(fileName); %%% new stlrad (adapt structure)
vertices = TR.Points;
...
rSph = Lx/2/nNodes; %Radius of potential spherical cells = grid density along X
...
%intersections of each ray with the collection of triangles in +x
...
for k = 1:nRayZ
    for j = 1:nRayY
        ...
        for facet=1:nface
            ...
            [flag, ~, ~, t] = rayTriangleIntersection(origin, direction, vert(1,:)', vert(2,:)', vert(3,:)');    %%% eps
            if(flag)
...
%Assume cubic elements (unique cells, no duplicate cells due to overlapping of spheres)
...
display(['Sphere Assembly Volume: ' num2str(totVolSph)]); 
...
%Shift the CoM to origin %%% no shifting
...
if(writeStlScaled)
    stlWrite(fullfile(filepath,[fName '_Scaled' ext]),faces, ...
        bsxfun(@minus,vertices,CoM'), ... % vertices, ...  %%% no scaling
